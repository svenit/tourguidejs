...
function goToStep(tgInstance: TourGuideClient, stepIndex: number) {
  return new Promise(async (resolve, bail) => {
    ...
    if (typeof nextStep.target === 'string')
      tgInstance.tourSteps[stepIndex].target = document.querySelector(nextStep.target as string);
    ...
    if (!nextStep.target || !tgInstance.tourSteps[stepIndex].target)
      tgInstance.tourSteps[stepIndex].target = document.body;
     ...
     * Scroll to target
     */
    if (tgInstance.options.autoScroll && nextStep.target !== document.body)
      await scrollToTarget(tgInstance, nextStep.target as HTMLElement);
    ...
    if (tgInstance._globalAfterChangeCallback && stepIndex !== currentStepIndex)
      await tgInstance._globalAfterChangeCallback();
...
export { goToStep, handleVisitNextStep, handleVisitPrevStep };
