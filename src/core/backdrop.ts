...
function computeBackdropAttributes(this: TourGuideClient) {
  ...
  this.backdrop.style.boxShadow =
    this.options.backdropColor + ' 0 0 0px 0px, ' + this.options.backdropColor + ' 0 0 0 1000vh';
  // Custom dialog class
  ...
  // Z-index
  if (this.options.backdropZ) {
    this.backdrop.style.zIndex = this.options.backdropZ.toString();
  }
}
...
function computeBackdropPosition(tgInstance: TourGuideClient) {
  return new Promise(async (resolve, reject) => {
    // TS build strict check
    if (typeof tgInstance.options.targetPadding === 'undefined')
      return reject('Options failed to initialize');
    ...
    const isOverflow =
      targetElemRect.width + tgInstance.options.targetPadding >
    ...
    if (targetElem === document.body) {
      ...
      tgInstance.backdrop.style.position = 'fixed';
      tgInstance.backdrop.style.top =
        targetElemRect.top - tgInstance.options.targetPadding / 2 + 'px';
      tgInstance.backdrop.style.left =
        (isOverflow ? targetElemRect.x : targetElemRect.x - tgInstance.options.targetPadding / 2) +
        'px';
    } else {
      tgInstance.backdrop.style.position = 'absolute';
      // Set position to match target
      tgInstance.backdrop.style.top =
        window.scrollY + targetElemRect.top - tgInstance.options.targetPadding / 2 + 'px';
      tgInstance.backdrop.style.left =
        (isOverflow ? targetElemRect.x : targetElemRect.x - tgInstance.options.targetPadding / 2) +
        'px';
    ...
    tgInstance.backdrop.style.width =
      (isOverflow
        ? targetElemRect.width
        : targetElemRect.width + tgInstance.options.targetPadding) + 'px';
    // Set height
    tgInstance.backdrop.style.height =
      (targetElemRect.height
        ? targetElemRect.height + tgInstance.options.targetPadding
        : targetElemRect.height) + 'px';
