...
const clickOutsideHandler = async function (this: TourGuideClient, event: MouseEvent) {
  ...
  if (
    event.clientX >= backdropRect.x &&
    event.clientX <= backdropRect.x + backdropRect.width &&
    event.clientY >= backdropRect.y &&
    event.clientY <= backdropRect.y + backdropRect.height
  )
    return;
  ...
  await this.onClickOutside?.(this);
};
