...
async function computeTourSteps(tgInstance: TourGuideClient) {
  return new Promise(async (resolve, reject) => {
     ...
     */
    const tourElements: NodeListOf<HTMLElement> =
      document.querySelectorAll("[data-tg-tour]");
    ...
    if (tourElements)
      tourElements.forEach((tourElem: HTMLElement) => {
        ...
        if (
          tgInstance.options.targetPadding &&
          tgInstance.options.autoScrollOffset
        ) {
          tourElem.style.scrollMargin =
            (scrollMargin
              ? scrollMargin + tgInstance.options.targetPadding
              : tgInstance.options.autoScrollOffset +
                tgInstance.options.targetPadding) + "px 0";
        } else {
          tourElem.style.scrollMargin =
            (scrollMargin ? scrollMargin : "30") + "px 0";
        }
     ...
     */
    if (tgInstance.group)
      computedSteps = computedSteps.filter((step: TourGuideStep) => {
    ...
    computedSteps.sort(function (a, b) {
      const keyA = new Date(a.order as number),
    ...
    if (!tgInstance.tourSteps.length)
      return reject(
        "No tour steps detected" +
          (tgInstance.group ? " in group: " + tgInstance.group : "")
      );
